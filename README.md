...
NCKU 計算機組織課程作業集。內容圍繞 **RISC-V ISA**:從手寫 inline assembly、使用 RISC-V V (Vector) Extension 做向量化加速,到修改 **Spike** 模擬器實作 FIFO cache replacement policy 與 cache-aware 程式優化。

> 📚 Course: NCKU Computer Organization 2025
...
- **GCC Inline Assembly** (`asm volatile` with extended operand constraints)
...
用 inline assembly 實作三種演算法,熟悉 RISC-V ISA 與 C/Assembly 介面:
...
- `arraymul_improved.c` — 用 **`vsetvli` + `vle32.v / vfmul.vv / vfadd.vf`** 向量化,實測加速 > 2×
...
- 修改 `riscv/cachesim.h` + `riscv/cachesim.cc`
...
針對 cache 規格(4-way / 8-set / 32B line)挑 block size = 8 × 8,讓單一 cacheline 內的資料一次處理完,大幅降低 conflict miss:
- Miss rate: 6.1% → 2.7%(transpose)、7.5% → 0.5%(multiply)
- Memory access overhead 改善 ~1.7× 與 ~3.5×
...
- Spike 原始碼閱讀與修改、重新編譯 toolchain
- Cache 行為對效能的實際影響,以及如何用 loop tiling / loop interchange 改善
