  ...
  fifo_queues.resize(sets);
	for (size_t i = 0; i < sets; ++i) {
	  for (size_t j = 0; j < ways; ++j) {
	    fifo_queues[i].push_back(j);  // 預先填好 FIFO 隊列
	  }
	}
...
{
  size_t idx = (addr >> idx_shift) & (sets - 1);
  size_t way = fifo_queues[idx].front();   // 拿最前面的一個
  fifo_queues[idx].pop_front();            // 移除它
  fifo_queues[idx].push_back(way);         // 加回隊尾 (因為這是新進資料)
...
uint64_t fa_cache_sim_t::victimize(uint64_t addr)
{
  ...
  tags[new_tag] = new_tag | VALID;
  fifo_queue.push_back(new_tag);
